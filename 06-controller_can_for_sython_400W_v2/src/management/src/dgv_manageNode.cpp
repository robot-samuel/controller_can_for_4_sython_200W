...
int main(int argc, char **argv)
{
    try{
        ...
        ROS_INFO("%s is start now,version=%s",NODENAME,VER);
          ...
          int canrecv_sensor_id;
           ...
           //初始化ROS_pub线程
           //外围传感参数配置
            Manager.rospub_mode_zmqreturn =  handel->advertise<std_msgs::String>("zmqreturn", 1);
            Manager.rospub_mode_uodateall =  handel->advertise<std_msgs::String>("dirv_Dcupdataall", 1);
	   Manager.rospub_mode_updateconfig=  handel->advertise<std_msgs::String>("updateconfig", 1);
	//(1)网络命令中转到驱动模块
	Manager.rospub_mode_net2ctrl =  handel->advertise<dgvmsg::DriverVelocity>("Net2Ctrl", 1);
	Manager.rospub_mode_net2Sctrl =  handel->advertise<std_msgs::String>("Net2SCtrl", 1);
	...
	handel->subscribe("Synthonfeedback", 80, &Dgv_Manage::Drivers_ros_cmdCall,&Manager);
