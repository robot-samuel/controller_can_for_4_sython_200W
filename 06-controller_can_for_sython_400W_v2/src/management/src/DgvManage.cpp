...
Dgv_Manage::Dgv_Manage()
{
            ...
            int index=1;
            zmqcmdlist.insert(std::make_pair("Heart_Beat",index++ ));
            ...
            zmqcmdlist.insert(std::make_pair("ctrlstop",index++));     //cmdstop  9
 ...
 void Dgv_Manage::BmsInfo_ros_cmdCall(const std_msgs::String::ConstPtr& msg)
{
	...
	if(Infostr.size()>10){ 
		//std::cout << "======BmsInfo_ros_cmdCall======"<<std::endl;
		try{
		BmsMessage msg;
		msg.ParseFromString(Infostr); 

		if(msg.has_stat()&&msg.has_bms_voltage()){
				statues.BmsModeInfo.CopyFrom(msg);
				//statues.BmsModeInfo.ParseFromString(Infostr);
 ...
 void Dgv_Manage::ExternInfo_ros_cmdCall(const std_msgs::String::ConstPtr& msg)
{
	// Verify that the version of the library that we linked against is
	// compatible with the version of the headers we compiled against.
	GOOGLE_PROTOBUF_VERIFY_VERSION;
 ...
 void Dgv_Manage::extern_zmq_cmdCall(const std_msgs::String::ConstPtr& msg)
{
	...
	std::string cmdstr = msg->data;
	if(cmdstr.size()>30){
			//std::cout << "======extern_zmq_cmdCall======"<<ros::Time::now()<<std::endl;
				try{
			proc_zmq_recvcmd(cmdstr);
				}
			catch(...){
				
				std::cout << "======extern_zmq_cmdCall===2==="<<std::endl;
			}		
		}
...
/*
std::wstring Dgv_Manage::strToWstr(const std::string& str){
	return boost::locale::conv::to_utf<wchar_t>(str, "GBK");
}

//UTF 2 GBK
std::string Dgv_Manage::wstrTostr(const std::wstring& wstr){
	return boost::locale::conv::from_utf(wstr, "GBK");
}

//Test boost::locale
void Dgv_Manage::test_locale(){
	std::wcout<<L"Test wchar:"<<std::endl;
	std::wcout<<strToWstr("一个中热刚发的撒垃圾")<<std::endl;

	std::cout<<"Test char:"<<std::endl;
	std::cout<<wstrTostr(L"家乐福大家了范德萨发生地方")<<std::endl;
}
*/
int Dgv_Manage::proc_zmq_recvcmd(std::string pack)
{
    ...
    if(strto.compare( ZMQ_NAME_DRIVERCTRL) ==0){
	     ...
	     if(funid!=1){
			...
			break;
		    case FUN_CTRL_STOP:
				//std::string  ss = boost::locale::conv::to_utf<char>( "收到停车指令", "GBK" );  
				//pack_audiohelp(ss);
...
{   
	...
	call->add_parameters(msg);
         ...
         std::cout << "ros_zmqupdate_SendRpc Interrupt exception was thrown." << std::endl;  
           packMSg.Clear();
	    packMSg.set_sessionid(get_session());		 
    }		
    ...
    std::string res;
       if (!packMSg.SerializeToString(&res)  ) {
           std::cerr << "Failed to write msg ros_zmqupdate_SendRpc." << std::endl; 
           packMSg.Clear();
	    packMSg.set_sessionid(get_session());
	    packMSg.SerializeToString(&res);
       }  	
...
std::string  Dgv_Manage::pack_dirv_Dcupdataal()
{
	...
	//ConTrolerstate.add_dirvermessage();
	//ConTrolerstate.add_dirvermessage();
	//ConTrolerstate.add_dirvermessage();
	...
	Pextern->CopyFrom(statues.ExternModeInfo);
	...
	ph3airmsg *ph3vau = ConTrolerstate.mutable_ph3air();
	ph3vau->CopyFrom(statues.Ph3ModeInfo);
	ph3vau->set_online(statues.Ph3SysNode.online);
