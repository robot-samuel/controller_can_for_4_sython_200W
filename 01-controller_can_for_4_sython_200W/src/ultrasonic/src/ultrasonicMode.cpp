...
ultrasonicSensorMode::ultrasonicSensorMode(std::string& port, unsigned int rate)
...
ultrasonicSensorMode::~ultrasonicSensorMode()
{
...
int ultrasonicSensorMode::open(std::string devname, unsigned int baudrate)
{
    try {
    ...
    pimpl->serial.setCallback(bind(&ultrasonicSensorMode::readCallback,this, _1, _2));
	return 1;
}

void ultrasonicSensorMode::close()
{
    pimpl->serial.clearCallback();
    try {
        pimpl->serial.close();
...
bool ultrasonicSensorMode::isOpen()
{
    return pimpl->serial.isOpen();
}

bool ultrasonicSensorMode::errorStatus()
{
    return pimpl->serial.errorStatus();
}

void ultrasonicSensorMode::write(std::string data)
{
    pimpl->serial.writeString(data);
}

void ultrasonicSensorMode::readCallback(const char *data, size_t size)
{
...
void ultrasonicSensorMode::ultrasonicSensorMode_master()
{
	
       std::cout<< "hello ultrasonicSensorMode_master "<<std::endl;
...
void ultrasonicSensorMode::print_dec(unsigned char *datBUF,int Num)
{
...
std::cout<<"\n";
 ...
 void ultrasonicSensorMode::print_hex(unsigned char *datBUF,int Num)
 {
 ...
 std::cout<<"\n";
  ...
  void ultrasonicSensorMode::print_hex(std::string str)
 {
	int Num = str.size();
	const char * datBUF =  str.c_str();
	
  std::cout<< "ultrasonicSensorMode print_recv hex:";
  for(std::size_t index=0;index<Num;index++){
                   int x=(unsigned  char)datBUF[index];
                    std::cout<<boost::format("%02X ")%x;
               }
...
int ultrasonicSensorMode::DriverecvDelay(  
 	int size,    
 	int timeout,    
 	std::string errprint){       
 	int count=0;    
...
void ultrasonicSensorMode::DevCmd_GetBmsInfo(){
...
void ultrasonicSensorMode::ros_SendRpc(std::string msg)
{   
    //ros::NodeHandle npub;
...
std::string ultrasonicSensorMode::packdata()
{
...
void ultrasonicSensorMode::printfinfo(BmsMessage &msg)
{
	
	  std::cout<< "ultrasonicSensorMode printfinfo:"<<std::endl;
