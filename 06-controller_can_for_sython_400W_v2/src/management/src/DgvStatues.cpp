...
Dgv_statues::Dgv_statues()
{
	...
	//过载错误
	Config_Error_Mode->set_currentoverloaderror_ischeck(false);
	...
	//Ph3ModeInfo.set_online(0);
	//Ph3ModeInfo.set_ph3_ppm(0);
	Ph3ModeInfo.Clear();
...
int Dgv_statues::IsCarCash(){
	...
	if(ExternModeInfo.io_collision(CRASH_IO)==0){
			//触发碰撞
...
int Dgv_statues::IsCarObstacle(){
	...
	if((ExternModeInfo.io_infrared(OBTC_LF)==0)||(ExternModeInfo.io_infrared(OBTC_RF)==0)||(Mode_PX24ES->out1()==0)
		||(Mode_PX24ES->out2()==0)){
			//触发前避障
...
int Dgv_statues::IsCarBObstacle(){
	...
	if(Config_Error_Mode->bobstaclestoperror_ischeck()==true){
	if((ExternModeInfo.io_infrared(OBTC_LB)==0)||(ExternModeInfo.io_infrared(OBTC_RB)==0)){
			//触发后避障
...
int Dgv_statues::IsCarDropStop(){
	...
	if(Config_Error_Mode->dropstoperror_ischeck() == true){
	if((ExternModeInfo.io_infrared(2)==1)||(ExternModeInfo.io_infrared(3)==1)){
			//触发防跌落
...
int Dgv_statues::IsCarEmergencyStop(){
	...
	if(Config_Alarm_Mode->emergency_status_stop_ischeck()==true){
            	...
            	if(feedback[0].emergencyinputvalue != 1){
	...
	if(Config_Alarm_Mode->ctrolboxhightemalarm_ischeck()== true){
		try{
			if(ExternModeInfo.temp_humi_size()>=2){
				...
				if(Mode_Temp_humi->interior_temp()>70){
...
int Dgv_statues::CheckAlarm_Ph3ReadNodeNdeOFF(){
	ParamConfig_Config_Alarm_Mode * Config_Alarm_Mode = config.mutable_config_alarm_msg();
	if(Config_Alarm_Mode->ph3readnodeoff_ischeck()== true){
			if(Ph3SysNode.online == 0){
					AlarmMsg.set_ph3readnodeoff(true);
				}
			else{
					AlarmMsg.set_ph3readnodeoff(false);

				}
		}

}
int Dgv_statues::CheckAlarm_Ph3ReadComLost(){

	ParamConfig_Config_Alarm_Mode * Config_Alarm_Mode = config.mutable_config_alarm_msg();
	if(Config_Alarm_Mode->ph3readcomlost_ischeck()== true){
			if(Ph3ModeInfo.online() == 0){
					AlarmMsg.set_ph3readcomlost(true);
				}
			else{
					AlarmMsg.set_ph3readcomlost(false);
...
void Dgv_statues::CheckAlarm(){
	...
	CheckAlarm_Ph3ReadNodeNdeOFF();
	CheckAlarm_Ph3ReadComLost();
...
void Dgv_statues::OnlineGroupInit(){
	...
	Ph3SysNode.online=0;
	Ph3SysNode.Set_timeout(5);

	ZmqTransNode.online=0;
	ZmqTransNode.Set_timeout(5);
...
void Dgv_statues::record_ExternModeInfo(std::string & str){
